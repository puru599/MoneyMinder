...
const IncompleteProfile = () => {
  ...
  const updateDetailsHandler = async (event) => {
    ...
    const response = await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyAle_pud5CBSRmol4VktTQSBgmBbnu0ZzQ",
      {
        method: "POST",
        body: JSON.stringify({
          idToken: idToken,
  ...
  useEffect(() => {
    const fillInputsHandler = async () => {
      const response = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyAle_pud5CBSRmol4VktTQSBgmBbnu0ZzQ",
        {
          method: "POST",
          body: JSON.stringify({
            idToken: idToken,
